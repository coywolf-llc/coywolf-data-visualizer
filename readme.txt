...
Turn CSV, Excel, and JSON data into Chart.js charts — built-in analyzer included, Claude optional — and embed them anywhere with a block.
...
2. (Optional) Go to Charts → Settings and paste a Claude API key for the best results.
...
Yes — that's the default. The built-in analyzer designs charts entirely on your server from the column types in your file. Adding a Claude API key unlocks the Claude engine, which produces the best results.
