...
final class Coywolf_CDV_GitHub_Updater {

	const REPO          = 'coywolf-llc/coywolf-data-visualizer';
	const TRANSIENT_KEY = 'coywolf_cdv_gh_release';
	const CACHE_HOURS   = 6;
	 ...
	 */
	const CRON_HOOK = 'coywolf_cdv_refresh_release';
	 ...
	 * "coywolf-data-visualizer/coywolf-data-visualizer.php".
	 *
	 * @var string
	 */
	...
	public function guard_pre_download( $reply, $package, $upgrader, $hook_extra = array() ) {
		...
		if ( ! is_string( $package ) || '' === $this->validate_package_url( $package ) ) {
			return new WP_Error(
				'coywolf_cdv_untrusted_package',
				__( 'Refusing to download a plugin update from an untrusted host.', 'coywolf-data-visualizer' )
			);
		}
		return $reply;
	}
		...
		foreach ( $plugin_meta as $i => $item ) {
			...
			if ( false !== strpos( $item, 'plugin-install.php?tab=plugin-information' )
				...
				$plugin_meta[ $i ] = sprintf(
					...
					esc_url( $repo_url ),
					esc_html__( 'View details', 'coywolf-data-visualizer' )
				);
			}
		}
		return $plugin_meta;
	...
	public function plugin_info( $result, $action, $args ) {
		...
		$info                = new stdClass();
		$info->name          = 'Coywolf Data Visualizer';
	...
	public function fix_source_dirname( $source, $remote_source, $upgrader, $hook_extra = array() ) {
		...
		return new WP_Error(
			'coywolf_cdv_rename_failed',
			__( 'Could not rename the downloaded update folder to match the plugin slug.', 'coywolf-data-visualizer' )
		);
	}
	...
	public function maybe_show_update_error() {
		...
		printf(
			...
			esc_html__( 'Coywolf Data Visualizer:', 'coywolf-data-visualizer' ),
			esc_html(
				sprintf(
					/* translators: %s: failure reason */
					__( 'could not check GitHub for updates (%s). This is usually temporary GitHub API rate-limiting on your host; the check retries automatically. If it persists, install the latest release manually from the plugin\'s GitHub Releases page.', 'coywolf-data-visualizer' ),
					$err
				)
			)
		);
