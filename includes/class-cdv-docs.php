...
final class Coywolf_CDV_Docs {
	...
	public static function render_page() {
		...
		if ( '' === $markdown ) {
			echo '<h1>' . esc_html__( 'Documentation', 'coywolf-data-visualizer' ) . '</h1>';
			echo '<p>' . esc_html__( 'The documentation could not be loaded.', 'coywolf-data-visualizer' ) . '</p>';
			echo '</div>';
			return;
		}
	...
	private static function read_readme() {
		$file = COYWOLF_CDV_PATH . 'readme.md';
	...
	public static function to_html( $md ) {
		...
		while ( $i < $n ) {
			...
			if ( preg_match( '/^```\s*([A-Za-z0-9_-]*)\s*$/', $line, $m ) ) {
				...
				$out .= '<pre class="coywolf-cdv-docs-code"><code' . $cls . '>' . esc_html( implode( "\n", $buf ) ) . '</code></pre>';
				continue;
			}
			...
			if ( false !== strpos( $line, '|' ) && isset( $lines[ $i + 1 ] )
				...
				$out .= '<table class="coywolf-cdv-docs-table widefat striped"><thead><tr>';
